...
use battlefield_rcon::{
    bf4::{error::Bf4Error, Bf4Client, Player, ReservedSlotsError},
...
use crate::guard::{
    ...
    Complement, Guard, InferFrom, Judgement,
};

use crate::guard::recent::Age;
...
impl Vips {
    ...
    pub async fn get(&self, name: &AsciiString, bf4: &Bf4Client) -> RconResult<Guard<AsciiString, Recent<MaybeVip>>> {
        let mut inner = self.inner.lock().await;
        inner.trim_old();
        ...
        Ok(self.refersh(bf4, |inner| {
            if let Some(j) = inner.vips.get(name) {
                unsafe {
                    Guard::new_raw(name.to_owned(), j.to_owned())
                }
            } else {
                let j = Recent::now(MaybeVip::right(NotVip));
                inner.vips.insert(name.to_owned(), j).unwrap();
                unsafe {
                    Guard::new_raw(name.to_owned(), j)
                }
            }
        }).await?)
    }

    async fn refersh<T>(&self, bf4: &Bf4Client, getter: impl FnOnce(&mut Inner) -> T) -> RconResult<T> {
        ...
        for reserved in reserved_list.iter() {
            if let Some(old) = inner.vips.insert(reserved.to_owned(), Recent::now(MaybeVip::left(YesVip))) {
